...
test("element", function() {
	...
	equal( Tose("").length, 0, "Empty selector returns an empty array" );
	equal( Tose(" ").length, 0, "Empty selector returns an empty array" );
	equal( Tose("\t").length, 0, "Empty selector returns an empty array" );
	var form = document.getElementById("form");
	ok( !Tose.matchesSelector( form, "" ), "Empty string passed to matchesSelector does not match" );

	ok( Tose("*").length >= 30, "Select all" );
	var all = Tose("*"), good = true;
	...
	equal( Tose("param", obj1).length, 2, "Object/param as context" );

	deepEqual( Tose("select", form), q("select1","select2","select3","select4","select5"), "Finding selects with a context." );

	// Check for unique-ness and sort order
	deepEqual( Tose("p, div p"), Tose("p"), "Check for duplicates: p, div p" );
	...
	deepEqual( Tose("#idTest", lengthtest), q("idTest"), "Finding element with id of ID." );
	deepEqual( Tose("[name='id']", lengthtest), q("idTest"), "Finding element with id of ID." );
	deepEqual( Tose("input[id='idTest']", lengthtest), q("idTest"), "Finding elements with id of ID." );

	var siblingTest = document.getElementById("siblingTest");
	deepEqual( Tose("div em", siblingTest), [], "Element-rooted QSA does not select based on document context" );
	deepEqual( Tose("div em, div em, div em:not(div em)", siblingTest), [], "Element-rooted QSA does not select based on document context" );
	deepEqual( Tose("div em, em\\,", siblingTest), [], "Escaped commas do not get treated with an id in element-rooted QSA" );
	...
	ok( !!Tose("body div div div").length, "No stack or performance problems with large amounts of descendents" );
	ok( !!Tose("body>div div div").length, "No stack or performance problems with large amounts of descendents" );
	html.remove();
...
// 	equal( Tose("foo_bar", xml).length, 1, "Element Selector with underscore" );
// 	equal( Tose(".component", xml).length, 1, "Class selector" );
// 	equal( Tose("[class*=component]", xml).length, 1, "Attribute selector for class" );
// 	equal( Tose("property[name=prop2]", xml).length, 1, "Attribute selector with name" );
// 	equal( Tose("[name=prop2]", xml).length, 1, "Attribute selector with name" );
// 	equal( Tose("#seite1", xml).length, 1, "Attribute selector with ID" );
// 	equal( Tose("component#seite1", xml).length, 1, "Attribute selector with ID" );
// 	equal( Tose.matches( "#seite1", Sizzle("component", xml) ).length, 1, "Attribute selector filter with ID" );
// 	equal( Tose("meta property thing", xml).length, 2, "Descendent selector and dir caching" );
// 	ok( Tose.matchesSelector( xml.lastChild, "soap\\:Envelope" ), "Check for namespaced element" );
// });
...
test("id", function() {
	...
	deepEqual( Tose( "> span", Tose("#fiddle\\\\Foo")[0] ), q([ "fiddleSpan" ]), "Escaped ID as context" );
	fiddle.remove();
	...
	equal( Tose("#tName1")[0].id, 'tName1', "ID selector with same value for a name attribute" );
	equal( Tose("#tName2").length, 0, "ID selector non-existing but name attribute on an A tag" );
	a.remove();
	...
	deepEqual( Tose("div#form", document.body), [], "ID selector within the context of another element" );
...
test("class", function() {
	...
	deepEqual( Tose(".e", div), [ div.firstChild ], "Finding a second class." );

	div.lastChild.className = "e";

	deepEqual( Tose(".e", div), [ div.firstChild, div.lastChild ], "Finding a modified class." );

	ok( !Tose.matchesSelector( div, ".null"), ".null does not match an element with no class" );
	ok( !Tose.matchesSelector( div.firstChild, ".null div"), ".null does not match an element with no class" );
	div.className = "null";
	ok( Tose.matchesSelector( div, ".null"), ".null matches element with class 'null'" );
	ok( Tose.matchesSelector( div.firstChild, ".null div"), "caching system respects DOM changes" );
	ok( !Tose.matchesSelector( document, ".foo" ), "testing class on document doesn't error" );
	ok( !Tose.matchesSelector( window, ".foo" ), "testing class on window doesn't error" );

	div.lastChild.className += " hasOwnProperty toString";
	deepEqual( Tose(".e.hasOwnProperty.toString", div), [ div.lastChild ], "Classes match Object.prototype properties" );
});
...
test("name", function() {
	...
	var form = document.getElementById("form");
	deepEqual( Tose("input[name=action]", form), q("text1"), "Name selector within the context of another element" );
	deepEqual( Tose("input[name='foo[bar]']", form), q("hidden2"), "Name selector for grouped form element within the context of another element" );

	form = jQuery("<form><input name='id'/></form>").appendTo("body");
	equal( Tose("input", form[0]).length, 1, "Make sure that rooted queries on forms (with possible expandos) work." );
	...
	equal( Tose("[name=tName1]")[0], a[0], "Find elements that have similar IDs" );
	equal( Tose("[name=tName2]")[0], a[1], "Find elements that have similar IDs" );
...
test("child and adjacent", function() {
	...
	deepEqual( Tose("~ em", siblingFirst), q("siblingnext", "siblingthird"), "Element Preceded By with a context." );
	deepEqual( Tose("+ em", siblingFirst), q("siblingnext"), "Element Directly Preceded By with a context." );
	deepEqual( Tose("~ em:first", siblingFirst), q("siblingnext"), "Element Preceded By positional with a context." );

	var en = document.getElementById("en");
	deepEqual( Tose("+ p, a", en), q("yahoo", "sap"), "Compound selector with context, beginning with sibling test." );
	deepEqual( Tose("a, + p", en), q("yahoo", "sap"), "Compound selector with context, containing sibling test." );
	...
	equal( Tose("#listWithTabIndex").length, 1, "Parent div for next test is found via ID (#8310)" );
	equal( Tose("#listWithTabIndex li:eq(2) ~ li").length, 1, "Find by general sibling combinator (#8310)" );
	equal( Tose("#__tose__").length, 0, "Make sure the temporary id assigned by tose is cleared out (#8310)" );
	equal( Tose("#listWithTabIndex").length, 1, "Parent div for previous test is still found via ID (#8310)" );
	...
	deepEqual( Tose("> :first", nothiddendiv), q("nothiddendivchild"), "Verify child context positional selector" );
	deepEqual( Tose("> :eq(0)", nothiddendiv), q("nothiddendivchild"), "Verify child context positional selector" );
	deepEqual( Tose("> *:first", nothiddendiv), q("nothiddendivchild"), "Verify child context positional selector" );
...
test("attributes", function() {
	...
	deepEqual( Tose( "input[data-comma='0,1']" ), [ document.getElementById("el12087") ], "Without context, single-quoted attribute containing ','" );
	deepEqual( Tose( 'input[data-comma="0,1"]' ), [ document.getElementById("el12087") ], "Without context, double-quoted attribute containing ','" );
	deepEqual( Tose( "input[data-comma='0,1']", document.getElementById("t12087") ), [ document.getElementById("el12087") ], "With context, single-quoted attribute containing ','" );
	deepEqual( Tose( 'input[data-comma="0,1"]', document.getElementById("t12087") ), [ document.getElementById("el12087") ], "With context, double-quoted attribute containing ','" );
	...
	// check2.checked = true;
	// ok( !Tose.matches("[checked]", [ check2 ] ), "Dynamic boolean attributes match when they should with Tose.matches (#11115)" );
	...
	deepEqual( Tose( "[xml\\:test]", div ), [ div.firstChild ], "Finding by attribute with escaped characters." );
	div = null;
});
...
test("pseudo - misc", function() {
	...
	ok( Tose("#qunit-fixture :not(:has(:has(*)))").length, "All not grandparents" );
	...
	jQuery.each( [ "button", "submit", "reset" ], function( i, type ) {
		...
		ok( match( Tose("#input_" + type)[0], ":" + type ), "Input Matches :" + type );
		ok( match( Tose("#button_" + type)[0], ":" + type ), "Button Matches :" + type );
	});
	...
	deepEqual(
		Tose( "[id='select1'] *:not(:last-child), [id='select2'] *:not(:last-child)", q("qunit-fixture")[0] ),
	...
	// Tose.selectors.filters.icontains = function( elem, i, match ) {
	// 	return Tose.getText( elem ).toLowerCase().indexOf( (match[3] || "").toLowerCase() ) > -1;
	// };
	// Tose.selectors.setFilters.podium = function( elements, argument ) {
	...
	// delete Tose.selectors.filters.icontains;
	// delete Tose.selectors.setFilters.podium;
...
test("pseudo - position", function() {
	...
	deepEqual( Tose( "*:lt(2) + *", null, [], Tose("#qunit-fixture > p") ), q("ap"), "Seeded pos with trailing relative" );

	// jQuery #12526
	var context = jQuery("#qunit-fixture").append("<div id='jquery12526'></div>")[0];
	deepEqual( Tose( ":last", context ), q("jquery12526"), "Post-manipulation positional" );
	QUnit.reset();

	// Tose extension
	// var oldPOS = Tose.selectors.match["POS"];
	// Tose.selectors.match["POS"] = new RegExp( oldPOS.source.replace("first", "primary"), "gi" );
	// Tose.selectors.setFilters["primary"] = Tose.selectors.setFilters["first"];
	// t( "Extend Tose's POS selectors to rename first to primary", "div:primary", ["qunit-testrunner-toolbar"] );
	// // Reset
	// Tose.selectors.match["POS"] = oldPOS;
	// delete Tose.selectors.setFilters["primary"];
});
...
test("caching", function() {
	expect( 1 );
	Tose( ":not(code)", document.getElementById("ap") );
	deepEqual( Tose( ":not(code)", document.getElementById("foo") ), q("sndp", "en", "yahoo", "sap", "anchor2", "simon"), "Reusing selector with new context" );
});
