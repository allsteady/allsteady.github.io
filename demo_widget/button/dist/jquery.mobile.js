...
$.widget( "mobile.button", {
	...
	_enhance: function() {
		var	$elem = this.element,
			isClass = /true/i.test( $elem.attr("data-byclass") );

		if ( isClass ) {
			// console.log("by Class type");
			$elem .wrap( this._classButton() );
		} else {
			$elem .wrap( this._button() );
		}
	},

	_classButton : function () {
		var	$elem = this.element,
			classes = $elem.attr( "class" ),
			tag = "<div class='ui-btn ui-input-btn ",
			matched, wrapper;

		matched = this._parseClass( classes );
		tag += matched.wrapperClasses;
		wrapper = $( tag + "' >" + $elem.val() + "</div>" );
		console.log ( tag + "' >" + $elem.val() + "</div> - 1"  );
		return wrapper;
	},

	_parseClass : function ( clazz )	 {
		...
		//set theme
		classes = clazz ? clazz : "";
		console.log ( "current class : [" + clazz  + "]");
		if ( matchedClass = classes.match(/ui-btn-[a-z]([ ]|$)/i) ) {
			result.wrapperClasses += " "+ matchedClass[ 0 ];
		}
		//set corner
		if ( matchedClass = classes.match(/ui-corner-all/i) ) {
			result.wrapperClasses += " ui-corner-all";
		}
		//set shadow
		if ( matchedClass = classes.match(/ui-shadow/i) ) {
			result.wrapperClasses += " ui-shadow";
		}
		//set inline
		if ( matchedClass = classes.match(/ui-btn-inline/i) ) {
		...
		if ( matchedClass = classes.match(/ui-state-disabled/i) ) {
			result.wrapperClasses += " ui-state-disabled";
		}
		// set icon
		if ( matchedClass = classes.match(/ui-icon-(action|alert|arrow-d-l|arrow-d-r|arrow-d|arrow-l|arrow-r|arrow-u-l|arrow-u-r|arrow-u|audio|back|bars|bullets|calendar|camera|carat-d|carat-l|carat-r|carat-u|check|clock|cloud|comment|delete|edit|eye|forbidden|forward|gear|grid|heart|home|info|location|lock|mail|minus|navigation|phone|plus|power|recycle|refresh|search|shop|star|tag|user|video)/i) ) {
			result.wrapperClasses += " "+ matchedClass[ 0 ];
		}
		// set iconpos
		if ( matchedClass = classes.match(/ui-btn-icon-(left|right|top|bottom|notext)/i) ) {
			result.wrapperClasses += " "+ matchedClass[ 0 ];
		}
		// set iconshadow
		if ( matchedClass = classes.match(/ui-shadow-icon/i) ) {
			result.wrapperClasses += " ui-shadow-icon";
		}
		return result;
	},
